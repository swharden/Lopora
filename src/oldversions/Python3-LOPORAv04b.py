...
WAVenabled = False          # A WAV audio snapshot is made and uploaded ONLY USE IT WHEN DOWNSAMPLING IS POSSIBLE!!!
...
SAMPLErate = 48000          # Sample rate of down sampled audio stream
ZEROpadding = 1             # Zero padding for interpolation between FFT points (power of 2, default 1, 0 for slow PC's, 2 for faster PC's)
ZOOMfactor = 1.22           # Zoom-out factor default 1, 2 for larger frequency range on screen
SMPfft = 131072             # Number of FFT samples, will be rounded off to nearest higher power of 2, (4096, 8192, 16384, 32768, 65536, 131072...)
FFTaverage = 1              # Average n traces, (2 to 10, 1 is off)
TUNEDfrequency = 10138464.0 # Tuning frequency of the receiver in Hz
SCREENupdate = 3            # Screen update per n FFT traces (3 to 10 to save processing time for screen update)
...
STARTfrequency = 10139840.0 # Start frequency, reception frequency at bottom of screen in Hz
NOISEblankerlevel = 0       # Noise blanker level (0 to 5, 0 is off)
DISPLAY = 2                 # Display compression factor 0 - 3
STACKING = 3                # Receive in 10 minute time synchronized intervals if STACKING > 0 and stack STACKING screens if > 1
############################################################################################################################################
...
def BStartfrequency():
    ...
    if (RUNstatus != 0):
        messagebox.showwarning("WARNING","Stop reception first")
        return()

    s = simpledialog.askstring("Start frequency: ","Value: " + str(STARTfrequency) + " Hz\n\nNew value:\n")
...
def Bsnapshot():
    ...
    SNAPshotenabled = messagebox.askyesno("Enable save snapshot", "Enable save snapshot?",default = "no")

    FTPenabled = messagebox.askyesno("Enable FTP upload", "Enable FTP upload?",default = "no")

    if FTPenabled == True:
        if FTPhost == "":
            FTPhost = simpledialog.askstring("FTP host: ","FTP host name:\n")

        if FTPuser == "":
            FTPuser = simpledialog.askstring("FTP user: ","FTP user name:\n")

        if FTPdir == "":
            FTPdir = simpledialog.askstring("FTP directory: ","FTP remote directory (press enter if none):\n")
                    
        if FTPpassword == "":
            FTPpassword = simpledialog.askstring("FTP password: ","FTP password:\n", show="*")
...
def SHOWtime():                                     # Show the reception time for one screen
    ...
    messagebox.showinfo("Reception time","Reception time for one screen:\n" + Ttxt + " minutes")     # show Time per screen in minutes 
...
def WAVrecord():                                            # Make audio snapshot and upload it with FTP
    ...
    RECORDtime = 3                                          # Seconds record time
    RATE = SAMPLErate                                       # The sample rate of the WAV file
    ...
    try:
        ...
        WAVenabled = False                                  # Just disable the WAV record function
        txt = "ERROR! Cannot record audio. WAVenabled is disabled."
        print(txt)
        return()

    if DEBUG > 0:
        txt = "Recording WAV file"
        print(txt)
...
def ASKWAVfilename():
    filename = filedialog.askopenfilename(filetypes=[("WAVfile","*.wav"),("allfiles","*")])
...
# ================ Make Screen ==========================
LoporaName = "LOPORAv04b.py(w) (18-12-2017): QRSS reception"
